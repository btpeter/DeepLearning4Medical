...
class ParameterServerModel():
   ...
   def __init__(self, x, y_, momentum, weight_cost_strength, variable_shapes, compute_gradients, apply_gradients, minimize, error_rate, session, batch_size):
      ...
      self.weight_cost_strength = weight_cost_strength
      ...
      self.variable_update = [None]*len(self.compute_gradients)
      ...
      for i in xrange(len(self.compute_gradients)):
          ...
          var_shape = variable_shapes[i]    
          self.variable_update[i] = np.zeros(var_shape)
          #self.variable_update[i] = np.zeros(gradient)
   ...
   def train(self, labels, features):                                                                                                                                      
      with self.session.as_default():                                                                                                                                      
        ...
        for i in range(len(self.compute_gradients)):       	                                                                                                                                                                  
        		grad = self.compute_gradients[i][0].eval(feed_dict=feed)
        		variable_update = self.variable_update[i]

        		# process variable update (add momentum)
        		variable_update = variable_update * self.momentum + (1. - self.momentum)*grad
        		self.variable_update[i] = variable_update

        		# process gradient update (add weight_cost_strength)
        		grad = variable_update + self.weight_cost_strength * variable_update
